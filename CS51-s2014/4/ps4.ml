...
(*>* Problem 2.2 *>*)
 ...
 (*   let (x,_) = pull_min t in let hd::_ = List.rev x in hd 
...
(*>* Problem 3.4 *>*)
  ...
  let test_add () =
      ...
      let x3 = C.generate_lt x2 () in
      let x4 = C.generate_lt x3 () in
      assert ((add x4 (add x (add x2 (add x3 empty)))) = 
(add x (add x4 (add x3 (add x2 empty)))))
  ...
  let test_take () = 
      ...
      let x3 = C.generate_lt x2 () in
      let x4 = C.generate_lt x3 () in
      assert (take (add x4 (add x (add x2 (add x3 empty)))) = (x4, (
(add x (add x3 (add x2 empty))))))

  let test_is_empty () =
    let x = C.generate () in
    assert (let (_,x) = (take (add x empty)) in is_empty x)
...
struct
  ...
  let test_add () =
      ...
      let x4 = C.generate_lt x3 () in
      assert (add x4 (add x (add x2 (add x3 empty))) = 
             (add x (add x2 (add x4 (add x3 empty)))))
  ...
  let test_take () = 
      ...
      let x3 = C.generate_lt x2 () in
      let x4 = C.generate_lt x3 () in
      assert (take (add x4 (add x4 (add x (add x2 (add x3 empty))))) = 
             (x4, (add x4 (add x (add x2 (add x3 empty))))))
...
struct
  ...
  let rec tree_size (t : tree) : int = 
    ...
    | Leaf _ ->  1
    | OneBranch (_,_) -> 2
    | TwoBranch (_,_,lt,rt) -> (tree_size lt) + 1 + (tree_size rt) 
  ...
  let rec fix (t : tree) : tree = 
    ...
    | Leaf e -> Leaf e
    | OneBranch (e, l) -> 
       (match C.compare e l with
	| Equal | Greater -> OneBranch (l, e)
	| Less -> OneBranch (e, l))

    | TwoBranch (e_o,e,t1,t2) ->
	(* check which tree's node is larger  *)
       (match C.compare (get_top t1) (get_top t2) with
	| Equal | Greater ->
	   (match C.compare e (get_top t2) with
	    | Greater -> 
	       (match t2 with
		| Leaf t2e -> TwoBranch (e_o, t2e, t1, Leaf e)
		| OneBranch (t2e, t2l) ->
		   TwoBranch (e_o, t2e, t1, (fix (OneBranch (e, t2l))))
		| TwoBranch (e_02, t2e, t2l, t2r) ->
		   TwoBranch (e_o,t2e, t1, 
			     (fix (TwoBranch (e_02, e, t2l, t2r))))
	       )
	    | Less | Equal -> TwoBranch (e_o, e, t1, t2))
	| Less -> 
	   (match C.compare e (get_top t1) with
	    | Greater ->
	       (match t1 with
		| Leaf t1e -> TwoBranch (e_o, t1e, Leaf e, t2)
		| OneBranch (t1e, t1l) ->
		   TwoBranch (e_o, t1e, fix (OneBranch (e, t1l)), t2)
		| TwoBranch (e_01, t1e, t1l, t1r) ->
		  TwoBranch (e_o, t1e, 
			     fix (TwoBranch (e_01, e, t1l, t1r)),t2)
	       )
	    | Equal | Less -> TwoBranch (e_o, e, t1, t2)))
  ...
  let rec get_last (t : tree) : elt * queue = 
    ...
    | TwoBranch (Even, x, lt, rt) -> let (e, ft) = get_last rt in
			  ( match ft with 
			    | Empty -> (e, Tree (OneBranch (x, get_top lt)))
			    | Tree tr -> (e, Tree (TwoBranch (Odd, x, lt, tr)))
			  )
    | TwoBranch (Odd, x, lt, rt) -> let (e, ft) = get_last lt in
			  (match ft with 
			   | Empty -> (e, Tree (OneBranch (x, get_top rt)))
			   | Tree tl -> (e, Tree (TwoBranch (Even, x, rt, tl)))
			    )
  ...
  let test_is_balanced (t : tree) : bool =
    match t with
    | Leaf _ -> true
    | OneBranch (_,_) -> true
    | TwoBranch (_,_,lt,rt) -> Int.abs (tree_size lt - tree_size rt) < 2
  ...
  let test_add () = 
      ...
      let x3 = C.generate_gt x2 () in
      let x4 = C.generate_gt x3 () in
      assert (add x (add x2 (add x3 (add x4 empty))) = 
	  (Tree (TwoBranch (Odd, x, OneBranch (x2, x4), Leaf x3))))

  let test_add_balanced () =
      let x = C.generate () in
      let x2 = C.generate_gt x () in
      let x3 = C.generate_gt x2 () in
      let x4 = C.generate_gt x3 () in
      let x5 = C.generate_gt x4 () in
      assert (test_is_balanced (extract_tree (
	         add x (add x2 (add x3 (add x4 (add x5 (add x empty))))))))
  ...
  let test_take_balanced () =
      let x = C.generate () in
      let x2 = C.generate_gt x () in
      let x3 = C.generate_gt x2 () in
      let x4 = C.generate_gt x3 () in
      let x5 = C.generate_gt x4 () in
      let (_,q) =  take (add x (add x2 (add x3 (add x4 (add x5 (add x empty))))))
      in assert (test_is_balanced (extract_tree (q)))
  ...
  let test_get_top () = 
      ...
      let x3 = C.generate_lt x2 () in
      let x4 = C.generate_lt x3 () in
      assert (get_top (extract_tree (add x4 (add x (add x2 (add x3 empty)))))
	      = x4)
  ...
  let test_fix () = 
      ...
      let x3 = C.generate_gt x2 () in
      let x4 = C.generate_gt x3 () in
      assert (fix (TwoBranch (Odd, x4, OneBranch (x, x2), Leaf x3)) = 
		(TwoBranch (Odd, x, OneBranch (x2, x4), Leaf x3)))
  ...
  let test_get_last () = ()
      ...
      let x3 = C.generate_gt x2 () in
      let x4 = C.generate_gt x3 () in
      assert (get_last (extract_tree (add x (add x2 (add x3 (add x4 empty))))) =
		(x4, add x (add x2 (add x3 empty))))
...
module IntBinHeap = BinaryHeap(IntCompare)

let _ = IntBinHeap.run_tests () 
...
(* this definition is not needed, it is defined above
module IntTreeQueue = (TreeQueue(IntCompare) :
                        PRIOQUEUE with type elt = IntCompare.t)
 *)
...
(* Tests each sort against the built in List.sort function.
 * Takes the number of tests you want and length of test lists. *)
let rec test_sorts (n : int) (list_len : int) : unit =
(* generate a len length list of random ints between -5000 and 4999 *)
  let rec gen_ran_list (len : int) : int list = 
    let lst = [] in 
    if len = 0 then lst 
    else ((Random.int 10000) - 5000)::(gen_ran_list (len - 1)) 
  in if n <= 0 then ()
     else
       let ranlist = (gen_ran_list list_len) in
       let sortedlist = (List.sort ~cmp:Int.compare ranlist) in
       let _ = assert( heapsort ranlist = sortedlist &&
     (selectionsort ranlist = sortedlist && treesort ranlist = sortedlist)) in
		       test_sorts (n-1) list_len
;;
...
module HeapGenSort(C : COMPARABLE) : SORT with type c = C.t =
  struct
    type c = C.t

    module HeapQueue = (BinaryHeap(C) : PRIOQUEUE with type elt = C.t)



    let sort  (lst : C.t list) =
       let rec extractor pq lst =
	if HeapQueue.is_empty pq then lst else
	  let (x, pq') = HeapQueue.take pq in
	  extractor pq' (x::lst) in
      let pq = List.fold_right ~f:HeapQueue.add ~init:HeapQueue.empty lst in
      List.rev (extractor pq [])

end

module IntHeapGenSort = (HeapGenSort(IntCompare) : SORT with type c = IntCompare.t);;

let rec test_g_sorts (n : int) (list_len : int) : unit =
  let rec gen_ran_list (len : int) : int list = 
    let lst = [] in 
    if len = 0 then lst 
    else ((Random.int 10000) - 5000)::(gen_ran_list (len - 1)) 
  in if n <= 0 then ()
       ...
       let ranlist = (gen_ran_list list_len) in
       let sortedlist = (List.sort ~cmp:Int.compare ranlist) in
       let _ = assert( IntHeapGenSort.sort ranlist = sortedlist ) in
		       test_g_sorts (n-1) list_len
