...
struct
  ...
  let rec fold (f: key -> value -> 'a -> 'a) (u: 'a) (d: dict) : 'a =
    ...
    | Three (l, (k1,v1), m, (k2,v2), r) ->
       f k1 v1 (f k2 v2 (fold f (fold f ( fold f u m ) l ) r)) 
  ...
  let string_of_dict (d: dict) : string = 
    let f = (fun k v y -> "\n key: " ^ D.string_of_key k ^
      "; value: (" ^ D.string_of_value v ^ ")" ^ y) in
    fold f "" d
  ...
  let insert_upward_two (w: pair) (w_left: dict) (w_right: dict)
    ...
    let (x_k,_),(w_k,_) = x,w in
    match D.compare w_k x_k with
    | Equal| Less -> Done (Three (w_left, w, w_right, x, x_other))
 ...
 let insert_upward_three (w: pair) (w_left: dict) (w_right: dict)
    ...
    let (x_k,_),(y_k,_),(w_k,_) = x,y,w in
       match D.compare w_k x_k, D.compare w_k y_k with
       | Equal,_|Less,_ -> Up (Two (w_left, w, w_right), x, 
			       Two (other_left, y, other_right))
       | Greater,Equal| Greater,Less -> 
			Up (Two (other_left, x, w_left), 
			    w, Two (w_right, y, other_right))
       | Greater,Greater -> Up (Two (other_left, x, other_right),
			y, Two(w_left, w, w_right))
  ...
  and insert_downward_two ((k,v): pair) ((k1,v1): pair)
    ...
    | Less ->
      (match insert_downward left k v with 
       ...
       | Up (l, k, r) -> insert_upward_two k l r (k1,v1) right
      )
    | Greater -> 
       (match insert_downward right k v with
	| Done (x) -> Done(Two(left, (k1,v1), x))
	| Up (l, k, r) -> insert_upward_two k l r (k1, v1) left
  ...
  and insert_downward_three ((k,v): pair) ((k1,v1): pair) ((k2,v2): pair)
	...
	| Up (l,k,r) -> insert_upward_three k l r (k1,v1) (k2,v2)
					    middle right
       )	
    | Greater, Less -> 
   ...
   * ANSWER:
   *   This function recursively walks down each branch of a tree, adding one
   *   to an accumulator until it hits a leaf. Once it hits the leaf, it starts
   *   returning, comparing each return to it's sibling node on the way up.
   *   If at any point there is a mismatch, it starts returning 0 which will
   *   propagate back up to the top and indicate there was a mismatch somewhere
   *   along the way. Else it returns the height of the tree, where a tree with
   *   only a leaf has a height of 1.
   *) 

  let balanced (d : dict) : bool =
    let rec treedepth (d: dict) (c: int) : int = 
    match d with
    | Leaf -> c
    | Two (l,_,r) -> let x,y = (treedepth l (c+1)),(treedepth r (c+1)) in if x = y then x else 0
    | Three (l,_,m,_,r)-> let x,y,z =(treedepth l (c+1)),(treedepth m (c+1)),(treedepth r (c+1))
			  in if x = y && y = z then x else 0

    in not( treedepth d 1 = 0)
  ...
  let test_fold () = 
    let plist = generate_pair_list 100 in
    let sdict = insert_list empty plist in
    let foldlist = fold (fun k _ a -> k::a) [] sdict in
    let _ = List.iter foldlist ~f:( fun k -> assert(member sdict k)) in
    assert(balanced sdict) ; ()

    

  let test_insert () = 
    let plist = generate_pair_list 5 in
    let sdict = insert_list empty plist in
    let sdict2 = insert_list_reversed empty plist in
    let _ =   Printf.printf "\nDictionary: %s \n" (string_of_dict sdict) in
    let _ =   Printf.printf "\nDictionary: %s \n" (string_of_dict sdict2) in
    let _ =   Printf.printf "\nString of tree: %s \n" (string_of_tree sdict) in
    let _ = assert(balanced sdict) in
    List.iter plist ~f:(fun (k,v) -> assert(lookup sdict k = Some v)) ;
    ()

  (* ensure double keys aren't inserted *)
  let test_insert2 () =
    let el0 = D.gen_key () in
    let el1 = D.gen_key_gt el0 () in
    let dict1 = insert (insert empty el1 (D.gen_value ())) el1 (D.gen_value ())
    in let _ = assert(balanced dict1)

					      
    in Printf.printf "\nInsert 2 test: \n %s" (string_of_dict dict1)

  let test_balance2 () = 
    let kv = D.gen_pair() in
    let two_leaf = Two(Leaf, kv, Leaf) in
    let d = Two( Two (two_leaf,kv,two_leaf ),kv,two_leaf )
    in assert(not (balanced d))
  ...
  let test_member () = 
    ...
    let dct = insert_list empty pairs in
    let _ = List.iter pairs ~f:( fun (k,_) -> assert(member dct k)) in
    assert(balanced dct) ; ()
  ...
  let test_choose () = 
    ...
    let dct = insert_list empty pairs in
    let ky, new_dct =
    match choose dct with
    | Some (k,_,d) -> k,d
    | None -> failwith "you had a bad time"
    in 
    let _ = assert(member dct ky && not(member new_dct ky)) in
     assert(balanced dct && balanced new_dct)

  let test_lookup () = 
    let pairs = generate_pair_list 100 in
    let dct = insert_list empty pairs in
    let _ = List.iter pairs ~f:( fun (k,v) -> assert(lookup dct k = Some v)) in
    assert(balanced dct) ; ()
