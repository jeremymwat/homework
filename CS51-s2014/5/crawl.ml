  ...
  if n = 0 || (LinkSet.is_empty frontier) then d else
    ...
    |Some (current_link, new_frontier) ->
     ( match get_page current_link with
      | Some cur_page -> 
	 crawl (n - 1)
	 (List.fold_right cur_page.links 
	 ~f:(fun lnk front -> if not(LinkSet.member visited lnk) 
			      then LinkSet.insert lnk front else front)
	 ~init: new_frontier)
	 (LinkSet.insert current_link visited)
	 (List.fold_right cur_page.words
	   ~f:(fun word dct -> 
	      match WordDict.lookup dct word with
	      | Some w -> WordDict.insert dct word 
					  (LinkSet.insert cur_page.url w)
	      | None -> WordDict.insert dct word 
					(LinkSet.singleton cur_page.url))
	   ~init: d)	       
