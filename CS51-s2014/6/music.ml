...
let rec list_to_stream (lst : obj list) : event stream =
  let rec list_to_stream_rec (nlst : obj list) : event stream =
    ...
    | [] -> list_to_stream lst
    | Note(p,d,v)::tl -> fun () ->
     Cons(Tone(0.,p,v), fun () -> Cons(Stop(d,p), list_to_stream_rec tl))
...
let rec pair (a : event stream) (b : event stream) : event stream =
  let h1,h2 = head a, head b in
  let tl1,tl2 = tail a, tail b in
  let d1,d2 = time_of_event h1,time_of_event h2 in
  let fix_next (t : float) (e : event) : event = 
    match e with
    | Tone(l,p,v) -> Tone(l -. t,p,v)
    | Stop(l,p) -> Stop(l -. t,p)
  in
  if d1 <= d2 then 
    fun () -> Cons(h1, pair tl1 (fun () -> Cons(fix_next d1 h2, tl2)))
  else fun () -> Cons(h2, pair (fun () -> Cons(fix_next d2 h1, tl1)) tl2)
...
let transpose (str : event stream) (half_steps : int) : event stream =
  let trans_event e = 
    match e with
    |Tone(l,p,v) -> Tone(l,transpose_pitch p half_steps,v)
    |Stop(l,p) -> Stop(l,transpose_pitch p half_steps)
  in map trans_event str
 ...
 * you're done. Run the program to hear the beautiful music. *)
 ...
 let canon = pair bass (pair (shift_start 2. melody) (pair (shift_start 4. melody) (shift_start 6. melody)));;
...
output_midi "part4.mid" 64 part4;;
