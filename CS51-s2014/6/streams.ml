...
(*>* Problem 2.1.e *>*)
let rec onest () = (*
  let _ = print_int ( headt (fun () -> Stem (1, onest, onest)) ) in *)
  Stem (1, onest, onest)
;;
...
let rec nth (n:int) (s:'a stream) : 'a =
  if n <= 0 then head s else 
    match s with
    | Cons(_, stm) -> nth (n-1) (Lazy.force stm)
;;
...
let rec merge (s1:int stream) (s2:int stream) : int stream =
  let rec nodupe (st:int stream) (prev:int) : int stream =
    let Cons(v, rs) = st in
    if v = prev then nodupe (Lazy.force rs) prev else st

  in let v1,v2 = head s1, head s2 in
  if v1 = v2 then Cons(v1, lazy(merge (nodupe s1 v1) (nodupe s2 v2)))
  else if v1 < v2 then Cons(v1, lazy(merge (nodupe s1 v1) s2))
  else Cons(v2, lazy(merge s1 (nodupe s2 v2)))
...
(*
 *  Answer: If one (or both) streams have an infinite number of repeating
 *          values (such as ones) merge will loop infinitely.
 *)
