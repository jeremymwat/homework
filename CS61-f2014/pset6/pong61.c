...
struct http_connection {
    ...
    int table_no;           // conn table index position
};
...
// return a new connection from the connection table

http_connection* get_connection(void) {
    // try to find connection to recycle or room for new connection

    while (1) { //TODO backoff?
        // lock access to the connection table
        pthread_mutex_lock(&mutex);
        for (int i = 0; i < 30; i++) {
            if (conn_table[i] == NULL) { // open slot
                conn_table[i] = http_connect(pong_addr);
                conn_table[i]->table_no = i;
                pthread_mutex_unlock(&mutex);
                return conn_table[i];
            } else if (conn_table[i]->state < -1) { //reuse broken conn
                conn_table[i] = http_connect(pong_addr);
                conn_table[i]->table_no = i;
                pthread_mutex_unlock(&mutex);
                return conn_table[i];
            } else if (conn_table[i]->state == HTTP_DONE) { //reuse done conn
                conn_table[i]->table_no = i;
                pthread_mutex_unlock(&mutex);
                return conn_table[i];
            }
...
//    fprintf(stderr, "called pong thread\n" );
    ...
    while (conn->status_code != 200) {
        ...
        usleep(backoff * 10000); //.01 seconds
        if (backoff < 128)
            backoff *= 2;
        conn = get_connection();
        http_send_request(conn, url);
...
static int http_process_response_headers(http_connection* conn) {
    ...
    // if header is too big, cut it down and fix end
    if (conn->len > BUFSIZ) {
        conn->len = BUFSIZ;
        conn->buf[BUFSIZ-2] = '\r';
        conn->buf[BUFSIZ-1] = '\n';
